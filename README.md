...
- **智能评分** — 语义匹配算法：同义词识别、模糊前缀匹配、编辑距离、虚词降权、内容词加权，中译英和英译中分别使用不同匹配策略
- **AI 评分** — 可接入 DeepSeek API 提供 AI 翻译质量评价（可选，默认关闭）
- **语法分析** — 自动检测主谓一致、a/an 误用、句首大写、双重否定等常见错误
- **题库管理** — 内置 100 句（雅思/四六级/考研/日常/学术），支持增删改、JSON 导入导出
- **练习统计** — 记录每日练习量、时长、连续打卡天数、每周图表
- **暗色模式** — 手动切换或自动跟随系统
- **PWA 支持** — 可添加到手机/电脑桌面，离线使用
...
纯前端单页应用，无框架依赖。数据存储在 localStorage，可选接入 DeepSeek API 进行 AI 评分。支持 PWA 离线使用。
